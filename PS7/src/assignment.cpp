...
pair<double, Intersection> Superquadric::ClosestIntersection(const Ray &ray) {
	...
	while (true) {
		...
		if (abs(g) < eps) {
			...
			// Transform to parent space and normalize location Ray
		    ...
		    location.direction = 
		    	transform_mat.inverse().transpose().block<3, 3>(0, 0) 
		    	* location.direction;
...
pair<double, Intersection> Assembly::ClosestIntersection(const Ray &ray) {
	...
	// Transform to parent space and normalize location Ray in Intersection
